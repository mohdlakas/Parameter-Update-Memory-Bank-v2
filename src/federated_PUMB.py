...
class PUMBFederatedServer:
    ...
    # def update_global_model(self, client_updates, client_losses, data_sizes):
    #     """Update the global model with client updates using PUMB.

    #     Args:
    #         client_updates: Dictionary of client parameter updates
    #         client_losses: Dictionary of (before, after) loss tuples for each client
    ...
    #     self.logger.info(f"=== ROUND {self.current_round} GLOBAL MODEL UPDATE ===")
    #     self.logger.info(f"Processing updates from {len(selected_clients)} clients")

    #     # Save current model state before update
    #     if self.prev_model_state is None:
    #         self.prev_model_state = self._get_model_state_copy()
    #         self.logger.info("Initialized previous model state")

    #     # Log client update statistics
    #     self._log_update_statistics(client_updates, client_losses, data_sizes)

    #     # Calculate aggregation weights
    #     self.logger.info("Calculating aggregation weights...")
    ...
    #     self.logger.info("Performing weighted aggregation...")
    #     self._aggregate_updates(client_updates, aggregation_weights)

    #     # Calculate global direction after update
    #     current_state = self._get_model_state_copy()
    #     self.global_direction = self._calculate_direction(
    #         self.prev_model_state,
    #         current_state
    #     )
    #     self._log_global_direction_stats()

    #     # Update memory bank with client contributions
    #     self.logger.info("Updating memory bank...")
    ...
    #     self.logger.info(f"Round {self.current_round - 1} completed successfully")
    #     return True

    # Fix 3: Update the aggregation method to handle full states
    def update_global_model(self, client_states, client_losses, data_sizes):
        """FIXED: Update using full client states, not parameter updates."""
        ...
        # aggregation_weights = self.client_selector.get_aggregation_weights(
        #     selected_clients,
        #     client_states,  # Pass full states
        #     data_sizes,
        #     self.global_direction,
        #     embedding_gen=self.embedding_gen,
        #     quality_calc=self.quality_calc
        ...
        aggregation_weights = self.client_selector.get_aggregation_weights(
            ...
            embedding_gen=self.embedding_gen,
            quality_calc=self.quality_calc,
            #pumb_alpha=getattr(self.args, 'pumb_alpha', 0.4)  # Pass the parameter
        )
        ...
        if self.prev_model_state is not None:
            ...
            # Update memory bank with parameter updates
            #self._update_memory_bank_with_updates(param_updates, client_losses, data_sizes)
    ...
    # def _aggregate_updates(self, client_updates, aggregation_weights):
    #     """Aggregate client updates with quality-aware weights."""
    #     # Reset gradients
    ...
    #     # Apply weighted updates directly to model parameters
    #     for name, param in self.model.named_parameters():
    #         weighted_update = torch.zeros_like(param.data)
    #         for client_id, weight in aggregation_weights.items():
    #             if name in client_updates[client_id]:
    #                 update = client_updates[client_id][name]
    ...
    #         update_norm = torch.norm(weighted_update).item()
    #         total_norm += update_norm
    #         param_count += 1

    #     avg_update_norm = total_norm / param_count if param_count > 0 else 0.0
    #     self.logger.info(f"Aggregation complete: avg_update_norm={avg_update_norm:.6f}, "
